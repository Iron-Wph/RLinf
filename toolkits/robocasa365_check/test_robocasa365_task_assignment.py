...
def _check_yaml_group_size_consistency(
    ...
    for label, path, expect_grpo in [
        ...
        if expect_grpo:
            ...
            if str(eval_env.get("episode_horizon_source", "max_episode_steps")) == (
                "task_horizon"
            ):
                eval_budget = int(eval_env.get("max_steps_per_rollout_epoch", 0))
                suite.check(
                    f"{label} task_horizon 配置给足 RoboCasa365 共享 rollout budget",
                    eval_budget >= 900,
                    f"max_steps_per_rollout_epoch={eval_budget}",
                )
